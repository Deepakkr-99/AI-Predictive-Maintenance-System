...
# Sahi working directory set karna takki file load error na aaye
...
# Custom style.css file link karna UI styling ke liye
css_file = Path(__file__).parent / "style.css"
st.markdown(f"<style>{css_file.read_text(encoding='utf-8')}</style>", unsafe_allow_html=True)

# Machine Learning trained model aur scaler files load karna
try:
    if "model" not in st.session_state:
        st.session_state.model = joblib.load("models/random_forest_model.pkl")
        st.session_state.scaler = joblib.load("models/scaler.pkl")
    model = st.session_state.model
    scaler = st.session_state.scaler
except FileNotFoundError:
    st.error("Model files load failed. Please run the training pipeline first.")
    st.stop()
...
try:
    ...
    st.error("Evaluation files are missing. Please run the training pipeline first.")
    st.stop()

# Dataset aur Model Results files read karna page charts ke liye
try:
    if "model_result" not in st.session_state:
        st.session_state.model_result = pd.read_csv("data/model_result.csv")
    if "feature_data" not in st.session_state:
        st.session_state.feature_data = pd.read_csv("data/feature_importance.csv")
    if "dataset" not in st.session_state:
        st.session_state.dataset = pd.read_csv("data/encoded_data.csv")
